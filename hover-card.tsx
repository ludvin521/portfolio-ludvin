...
import type { ResearchProject } from '../types/config';
    ...
    >
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        ...
        <div
          ...
          style={{ gap: 0 }}
        >
          {researchConfig.projects.map((project: ResearchProject, i: number) => (
            <div
              key={`${project.title}-${i}`}
              ref={(el) => { itemRefs.current[i] = el; }}
              className="group cursor-pointer"
              onClick={() => navigate(`/realisation/${realisationSlugs[i]}`)}
              style={{
                borderBottom: '1px solid rgba(255, 255, 255, 0.1)',
                borderRight: (i + 1) % 5 !== 0 ? '1px solid rgba(255, 255, 255, 0.1)' : 'none',
                padding: '24px 20px',
              }}
            >
              <div
